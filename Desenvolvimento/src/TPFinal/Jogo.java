...
public class Jogo implements InterfaceProcessingApp {
	...
	public void draw(PApplet p, float dt) {
		if (!ms.isStartGame()) {
			...
			// caso o boss exista, fa�o spawn dele aqui:
			if (boss != null) {
			...
			for (Osso osso : ossos) {
				...
				for (Inimigo inimigo : inimigos) {
					...
					if (osso.getBody().collision(inimigo.getBody(), plt)) {
						...
						if (currentInimigoHP <= 1) { // por alguma raz�o se for 0 ele d� mais 1 de vida aos inimigos
							float min = 0, max = 1;
						...
						// mesmo que o inimigo n�o desapare�a, o osso � sempre removido!
				...
				if (boss != null && osso.getBody().collision(boss.getBody(), plt)) {
					// System.out.println("Colis�o osso Boss!");
					...
					if (currentBossHP <= 1) { // por alguma raz�o se for 0 ele d� mais 1 de vida aos inimigo
						boss.setFlagRemove(true);// inimigop tamb�m tem que ser removido no proximo draw!
						bossFight = false;
						HS = HS + 50;
					}
	...
	public void decideWhatPRW() {
		...
		// 15% para diminuir o tempo entre disparos. tenho que garantir que o valor �
		// sempre positivo!
	...
	public void mousePressed(PApplet p) {
		...
		if (isInsideRect(p.mouseX, p.mouseY, startGameRect[0], 200, startGameRect[1], 50) && !ms.isStartGame()
			...
			resetEverything(); // cada vez que carrego no start game, come�o o jogo de novo!
