...
public class DynamicJulia implements InterfaceProcessingApp {
	...
	public void draw(PApplet p, float dt) {
		...
		if (drawJulia) {
			...
			// o codigo para o julia set e praticamente igual so que soma um numero
			// especificio para dar o efeito que quero
			...
			for (int j = 0; j < p.height; j++) {
				...
				for (int i = 0; i < p.width; i++) {
					...
					while (n < maxiterations) {
						// We suppose z = a+ib
						// isto aqui dentro e aquela matematica toda complexa que esta no video e que o
						// prof explicou
						float aa = a * a;
						float bb = b * b;
