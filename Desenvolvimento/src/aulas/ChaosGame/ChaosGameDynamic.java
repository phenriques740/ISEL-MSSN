	...
	public void draw(PApplet p, float dt) {
		...
		for (int i = 0; i < arrPontos.length; i++) {
			if ((arrPontos[i][0] != 0) && (arrPontos[i][1] != 0)) { // se ambas as coordenadas forem diferentes de 0
				// tenho que fazer isto porque, o array e inicilizado com ZEROS inv�s de
				// null....
				p.circle((float) arrPontos[i][0], (float) arrPontos[i][1], 4);
				lastValidIndex = i;
			}
			if (i >= 1 && ((arrPontos[i][1] != 0)) && (arrPontos[i][0] != 0)) {
				p.line((float) arrPontos[i - 1][0], (float) arrPontos[i - 1][1], (float) arrPontos[i][0],
						(float) arrPontos[i][1]);
			}
		}
		if (rightPressed) {
			p.line((float) arrPontos[0][0], (float) arrPontos[0][1], (float) arrPontos[lastValidIndex][0],
					(float) arrPontos[lastValidIndex][1]);
