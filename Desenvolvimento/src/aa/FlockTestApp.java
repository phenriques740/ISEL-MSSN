...
public class FlockTestApp implements IProcessingApp {
	...
	public void draw(PApplet p, float dt) {
			...
			if (posToDestroy != null) {
				...
				if (difference.mag() < .5f) {
					if (b.equals(targetBoid)) {
						// se carregar no target do meu predador, tenho que lhe por os comportamentos
						// correctos!
		...
		// Codigo para o Boid especial e o seu target. o meu boid especial � um
		// predador!
		boidSpecial.applyBehaviors(dt);
		if (!targetBoid.isFlagRemove()) {
			// apenas vejo a visibilidade da presa Boid se estiver activa! se ja tiver sido
			// removido nao preciso de ver
			targetBoid.getEye().display(p, plt);
		}
		// boidSpecial.applyForce();
		boidSpecial.display(p, plt);

		// se o meu predador estiver praticamente em cima da presa, marco a presa para
		// ser removida,e escolho um novo target, adiciono ao eye do predador!
		...
		if (differenceBoid.mag() < 1f && (!targetBoid.isFlagRemove())) {
			...
			// a flag de cima e absolutamente necess�ria, caso contr�rio o boidEspecial cada
			// vez que passa na posi��o antiga da presa,
		...
		if (showTips) {
			p.text("LMB/RMB para destruir Boid's. Quando um Boid � destruido desta maneira, crio um sistema de particulas durante poucos segundos",
					20, 50);
			p.text("Boid vermelho persegue o Boid que tem a vis�o. Ap�s o Boid predador apanhar a presa, a presa � destruida",
					20, 70);
			p.text("Enquanto o Boid n�o tem um novo targer faz Wander. Para dar um novo target carrego T. para esconder mensagens H",
					20, 90);
