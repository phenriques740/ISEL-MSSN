...
public class SpriteAnimated implements InterfaceProcessingApp {
	...
	private float MCStartingVel = 0.5f;
	...
	public void setup(PApplet p) {
		...
		for (int i = 0; i < 5; i++) {
			// vou instanciar o meu boneco principal na posi��o especificada. util ter isto
		...
		MC = new SpriteDef(animationMC, MCStartingPos[0], MCStartingPos[1], MCStartingVel, p);	//a MCStartingVel tem os valores iguais em ambas as componentes, nao faz diferen�a entr [0] e [1]
		
		plt = new SubPlot(window, viewport, p.width, p.height);
		PVector worldCoordToPlt = new PVector( (float)plt.getWorldCoord(MCStartingPos)[0] , (float)plt.getWorldCoord(MCStartingPos)[1]  ); //fazer a convers�o de coordenadas do mundo (em pixeis) para
	...
	public void draw(PApplet p, float dt) {
		...
		MCBody.move(dt*15);	//este * 15 e porque na classe da sprite multiplico por 10 para ser mais r�pido, para acompanhar a anima��o ponho 15
