...
public class ParticleSystemApp implements InterfaceProcessingApp {
	...
	private double[] window = { -10, 10, -10, 10 }; // x entre -10 e 10| y entre 10 e -10. TODOS OS VALORES T�M QUE
	...
	public void setup(PApplet p) {
		...
		// ps = new ParticleSystem(new PVector(), new PVector(), 1f, .2f,
		// p.color(255,0,0), 1f, new PVector(-5,-5) );
	...
	public void draw(PApplet p, float dt) {
		...
		/*
		 * //aplicar uma for�a tipo peso: for(ParticleSystem ps : pss) {
		...
		/*
		 * //as liinhas de baixo permitem que se mexer o rato, a direcao dos
		 * particleSystems � alterada! velParams[0] = PApplet.map(p.mouseX, 0, p.width,
		 * PApplet.radians(0), PApplet.radians(360) ); for(ParticleSystem ps : pss) {
		 * //percorrer todos os particleSystem's que ja foram instanciados PSControl psc
		 * = ps.getPSControl(); psc.setVelParams(velParams); }
		 */
